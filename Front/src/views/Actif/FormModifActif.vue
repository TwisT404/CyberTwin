<template>
  <div class="page">
    <section class="card">
      ...
      <form v-else @submit.prevent="modifierActif()">
...
export default {
  ...
  methods: {
    async chargerActif() {
      ...
      try {
        const data = await actifStore.fetchActifById(this.$route.params.id)
        this.actifs = {
          actif_nom: data.actif_nom,
          type: data.type,
          criticite: data.criticite,
          est_expose_internet: !!data.est_expose_internet,
    ...
    async modifierActif() {
      ...
      try {
        await actifStore.modifierActif(this.$route.params.id, this.actifs)
        alert('Actif modifié');
